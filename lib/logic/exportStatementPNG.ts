...
export const generateStatementCanvas = (group: Group, isDark: boolean = true): HTMLCanvasElement | null => {
...
export const exportStatementPNG = (group: Group, isDark: boolean = true) => {
    const canvas = generateStatementCanvas(group, isDark);
    if (!canvas) return;
...
export const shareStatementPNG = async (group: Group, isDark: boolean = true) => {
    const canvas = generateStatementCanvas(group, isDark);
    if (!canvas) return;

    canvas.toBlob(async (blob) => {
        if (!blob) {
            alert("Failed to generate statement image.");
            return;
        }
        const file = new File([blob], `${group.name.replace(/\s+/g, "_")}_statement.png`, { type: "image/png" });
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
            try {
                await navigator.share({
                    title: `${group.name} Statement`,
                    text: `Here is the expense statement for ${group.name}.`,
                    files: [file]
                });
            } catch (error) {
                // User cancelled or share failed — degrade gracefully without alarming the user
                console.error("Share aborted or failed:", error);
            }
        } else {
            alert("Your device or browser does not support native file sharing. Please use the Export Statement button to save it locally.");
        }
    }, "image/png");
};
